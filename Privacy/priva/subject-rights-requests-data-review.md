...
**[データの確認**] ステージは、コラボレーターが [**データ収集**] タブでコンテンツ 項目を調べる場合です。すべての関係者によるコンテンツレビューを容易にするために、Teams チャネルが自動的に設定されます。 詳細については、以下 [のデータ レビューのコラボレーション](#collaboration-for-data-review) を参照してください。 データ レビュー 手順の重要なタスクの概要を次に示します。

#### <a name="import-additional-files"></a>その他のファイルをインポートする

データ レビューの要求に追加のコンテンツ アイテムを取り込む必要がある場合があります。 たとえば、組織のMicrosoft 365環境の外部に保存されているファイルや、関連性が高いと思うが検索の範囲が設定されていない他のアイテムなどです。 ファイルは、他の項目と共に、確認および操作する個々の要求の [ **データ収集** ] タブにインポートできます。 インポートされたファイルは、検索から取得した他のコンテンツ アイテムと同じAzure Blob Storage コンテナーに追加されます。

ファイルをインポートするには、次の手順に従います。

1. 要求の詳細ページで、ページの上部にあるコマンド バーから **[ファイルのインポート** ] を選択します。

2.  [ **ファイルのインポート** ] ボックスが画面に表示されます。 [ **ファイルの選択] を選択** し、エクスプローラー ビューからインポートする 1 つ以上のファイルを選択します。

3. [ **ファイルのインポート** ] ボックスに戻り、選択したファイルの一覧が表示されます。 [ファイルの **選択]** をもう一度選択すると、リストにアイテムを追加できます。 いずれかのファイルを削除するには、[ **クリア**] を選択し、すべてのファイルを削除し、もう一度ファイルを選択します。

4. 必要なすべてのファイルが [ **インポート** ファイル] ボックスに一覧表示されたら、[ **インポート**] を選択します。 アップロードせずに終了するには、[インポートの **キャンセル]** を選択します。

インポートが開始されると、要求の [ **データ収集] タブに** 戻ります。タブの上にあるメッセージは、アップロードが進行中であることを示します。 アップロードに問題がある場合は、メッセージによって通知され、再試行するオプションが提供されます。

インポートが完了すると、[ **データ収集]** タブの上に確認メッセージが表示されます。

**ファイルのインポートの詳細:**

- 個々のファイル サイズは 4 MB を超えることはできません。 ファイルが大きすぎてアップロードできない場合、[ **ファイルのインポート** ] ボックスに警告メッセージが表示されます。

- インポートされたファイルが [ **データ収集** ] タブで使用できるようになるまでに、最大で 20 分かかる場合があります。

- 1 人のユーザーに対してインポートが既に進行中の場合、同じユーザーは、前のアップロード プロセスが完了するまで、追加のファイルをアップロードできません。 複数のユーザーが同じ要求に同時にファイルをアップロードできます。 ただし、進行中のアップロードが多いほど、完了に時間がかかります。 要求の状態メッセージは、アップロードが完了し、ファイルを確認する準備が整ったときに通知されます。

#### <a name="mark-items-as-include-or-exclude-and-add-notes"></a>アイテムを [含める] または [除外] としてマークし、メモを追加する

要求の [ **データ収集** ] タブに一覧表示されている項目を確認します。 アイテムを最終的なレポートの一部としてデータ主体に戻す必要があると判断した場合は、アイテムの一覧の上部にあるコマンド バーで **[含める** ] を選択します。 項目の一覧の右側にあるコンテンツ レビュー領域で青い **[インクルード** ] ボタンを選択することもできます。 **[含める**] を選択すると、ポップアップ ウィンドウが表示され、メモを追加するオプションが表示されます。 完了したら、[ **送信]** を選択して、アイテムのレビュー状態を **[含める**] として保存します。

アイテムが要求の一部として属していない場合は、コマンド バーで **[除外** ] を選択するか、コンテンツ レビュー領域の [ **除外]** ボタンを選択できます。 項目を **除外として明示的** にマークすることは、多くの場合、内部レコードに必要です。

既定では、[データ主体に対して生成される最終レポート](subject-rights-requests-reports.md)には **、[含める**] としてマークした項目のみが含まれます。

> [!NOTE]
> アイテムを **除外** としてマークする場合は、件名の権利要求に関連しない理由を理由としてメモを追加する必要があります。 メモは内部目的のためであり、最終レポートには含まれません。

コンテンツが検索クエリの誤検知と見なされる場合は、[ **一致しない** ] を選択し、ポップアップ ウィンドウで [ **確認**] を選択します。 このアクションは、アイテムに対して、検索で検出すべきでないものとしてフラグを設定します。
...
サブジェクト権利要求の右上隅にある設定をPrivaすることで、サブジェクト権利要求の **Teams** チャネルを作成する既定の動作を変更できます。 **Teamsコラボレーション** を選択し、ページのボックスをオフにして、すべてのサブジェクト権限要求のTeams機能をオフにします。
